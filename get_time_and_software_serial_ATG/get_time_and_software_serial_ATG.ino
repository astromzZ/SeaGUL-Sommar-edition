/*
 ...
 Example: Get Time with Software Serial Example
 ...
 ** (The Artemis Module does not have a Wire port defined, which prevents the IridiumSBD library from compiling) **
 ...
 This example powers up the Iridium 9603N and retrieves the Iridium system time
 ...
 The 9603N's Network Available signal is connected to D18,
 so let's configure D18 as an input.

 The 9603N's Ring Indicator is connected to D41,
 so let's configure D41 as an input.

 To prevent bad things from happening to the AS179 RF antenna switch,
...
SoftwareSerial mySerial(D5, D6); // RX, TX - Any pins can be used
...
void IridiumSBD::endSerialPort()
{
  ...
  am_hal_gpio_pinconfig((PinName)25, g_AM_HAL_GPIO_DISABLE);
}
...
void setup()
{
  ...
  Serial.println(F("Example: Get Time with Software Serial"));
  Serial.println();
...
void loop()
{
    ...
    if (err != ISBD_SUCCESS)
    {
      Serial.print(F("[Iridium] SignalQuality failed: error "));
      Serial.println(err);
      return;
    }
  
    Serial.print(F("[Iridium] On a scale of 0 to 5, signal quality is currently "));
    ...
    if (err == ISBD_SUCCESS)
    {
      ...
      Serial.print(F("[Iridium] Iridium date/time is "));
      Serial.println(buf);
      // Send the date/time to the ESP via Software Serial
      mySerial.print("Iridium date/time is ");
      mySerial.println(buf);
    }
    else if (err == ISBD_NO_NETWORK) // Did it fail because the transceiver has not yet seen the network?
    {
      Serial.println(F("[Iridium] No network detected. Waiting 5 seconds."));
      // Send the network status to the ESP via Software Serial
      mySerial.println("No network detected. Waiting 5 seconds.");
    }
    else
    {
      Serial.print(F("[Iridium] Unexpected error "));
      Serial.println(err);
      // Send the error to the ESP via Software Serial
      mySerial.print("Unexpected error ");
