...
- The steppermotors can be configured for microstepping (see [2. Electronics](2.Electronics.md)). In the gliders current state the microstepping is set to 1/16 which allows for smooth motion and precise movement. When utilising the max range on the translaitonal movement, the values therefore go from 0 - 6200\*16 = 0 - 92000 steps. The rotaitional motors go \+/\- 250\*16 steps.
